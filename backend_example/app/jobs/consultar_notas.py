    ...
    with get_db_connection() as conn:
        ...
        cur.execute("""
            SELECT 
                ...
                e.data_envio,
                e.status_arquivo,
                e.trello_card_criado
            FROM envios_montagem e
            WHERE e.link_upload IS NOT NULL
            AND (e.upload_hash IS NOT NULL OR e.id_controle IS NOT NULL)
            AND (
                (e.status_api IS NULL OR e.status_api < 2)
                OR (e.status_arquivo = 2 AND (e.trello_card_criado IS NULL OR e.trello_card_criado = FALSE))
            )
...
def salvar_nota_fiscal_montagem(envio_id: int, caminho_arquivo: str):
    ...
    with get_db_connection() as conn:
        ...
        # Atualizar envio com todas as informações (incluindo status texto)
        cur.execute("""
            UPDATE envios_montagem
            SET 
                nota_fiscal_path = %s,
        ...
        logger.info(f"   ✅ Status atualizado para 'N.F RECEBIDA'")
        logger.info(f"   📅 Data vencimento: {data_vencimento.strftime('%d/%m/%Y')} ({dias_vencimento} dias)")
    ...
    try:
        ...
        for envio in envios_montagem:
            ...
            if len(arquivos) > 0:
                ...
                # Verificar se já foi processado (arquivos baixados)
                ...
                if ja_processado and trello_card_criado:
                    # Totalmente processado, pode pular
                    logger.info(f"   ℹ️  Arquivos já baixados E card Trello já criado - pulando")
                    continue
                
                if not ja_processado:
                    # Ainda não baixou, processar download
                    logger.info(f"   🔄 Processando download de arquivos...")
                    ...
                    # Criar notificação e enviar WhatsApp (só no download fresco)
                    if not ja_tem_notificacao:
                        total_arqs = len(arquivos)
                        ...
                        try:
                            ...
                            enviar_notificacao_websocket_sync(
                                ...
                                dados={
                                    ...
                                    "total_arquivos": total_arqs,
                                    "tamanho_total": stats['total_tamanho_formatado'],
                                    "nota_fiscal": os.path.join(pasta_destino, arquivos[0]['nome_original'])
                                }
                            )
                            logger.info(f"   📡 Notificação WebSocket enviada")
                    ...
                    logger.info(f"   ℹ️  Arquivos já baixados, verificando card Trello...")
                
                # Integração Trello - criar card (verificar se já foi criado)
                # VERIFICAÇÃO COM LOCK para evitar race condition
                with get_db_connection() as conn_check:
                    cur_check = conn_check.cursor()
                    # SELECT FOR UPDATE para lock na linha
                    cur_check.execute("""
                        SELECT trello_card_criado 
                        FROM envios_montagem 
                        WHERE id = %s 
                        FOR UPDATE
                    """, (envio_id,))
                    result = cur_check.fetchone()
                    trello_card_criado = result[0] if result else False
                    
                    if not trello_card_criado:
                        # Marcar como "em processamento" imediatamente para evitar duplicação
                        cur_check.execute(
                            "UPDATE envios_montagem SET trello_card_criado = TRUE WHERE id = %s AND trello_card_criado = FALSE",
                            (envio_id,)
                        )
                        rows_affected = cur_check.rowcount
                        conn_check.commit()
                        
                        if rows_affected == 0:
                            # Outra thread já marcou, pular
                            logger.info(f"   ℹ️  Card Trello já sendo criado por outra instância")
                            continue
                        ...
                        try:
                            ...
                            if trello.is_configured():
                                ...
                                if card_result:
                                    ...
                                    cur_check.execute(
                                        "UPDATE envios_montagem SET trello_card_criado = FALSE WHERE id = %s",
                                        (envio_id,)
                                    )
                                    conn_check.commit()
                            else:
