...
def refresh_token():
    ...
    with get_db_connection() as conn:
        ...
        try:
            # Usar API REST diretamente para renovar (mais confiável que MSAL)
            ...
            if response.status_code == 200:
                ...
                raise HTTPException(
                    status_code=401, 
                    detail=f"Erro ao renovar token: {error_data.get('error_description', 'Falha na renovação')}"
                )
...
def get_valid_access_token() -> str:
    ...
    with get_db_connection() as conn:
        ...
        # Token expirado ou próximo de expirar, tentar renovar usando refresh_token
        if auth.get("refresh_token"):
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(f"✅ Token Microsoft renovado automaticamente. Válido até: {new_expires_at}")
